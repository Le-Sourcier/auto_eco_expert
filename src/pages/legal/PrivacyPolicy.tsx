...
import { ArrowLeft, Shield, Lock, Eye, RefreshCw, UserX, Database } from 'lucide-react';
...
const PrivacyPolicy = () => {
  ...
  return (
    <>
      <Helmet>
        <title>{t('privacy.meta.title')} - Auto Éco Expert</title>
            ...
            <div className="relative">
              <div className="flex items-center gap-4 mb-6">
                <div className="bg-[var(--primary-50)] p-3 rounded-full">
                  <Shield className="w-8 h-8 text-[var(--primary-600)]" />
                </div>
              ...
              <div className="prose max-w-none">
                {t('privacy.sections', { returnObjects: true }).map((section: any, index: number) => (
                  <div key={index} className="mb-12 last:mb-0">
                    <div className="flex items-center gap-3 mb-4">
                      {index === 0 && <Lock className="w-6 h-6 text-[var(--primary-600)]" />}
                      {index === 1 && <Database className="w-6 h-6 text-[var(--primary-600)]" />}
                      {index === 2 && <Eye className="w-6 h-6 text-[var(--primary-600)]" />}
                      {index === 3 && <Shield className="w-6 h-6 text-[var(--primary-600)]" />}
                      {index === 4 && <UserX className="w-6 h-6 text-[var(--primary-600)]" />}
              ...
              <div className="mt-12 pt-8 border-t border-gray-100">
                <div className="flex items-start gap-4 bg-[var(--primary-50)] rounded-xl p-6">
                  <div className="bg-white p-3 rounded-full shrink-0">
                    <Shield className="w-6 h-6 text-[var(--primary-600)]" />
                  </div>
                  <div>
                    ...
                    <p className="text-[var(--primary-600)]">
                      ...
                      contactez notre délégué à la protection des données à privacy@auto-eco-expert.fr
                    </p>
                  </div>
                </div>
              </div>
